...
class COMPQProcessor(DataProcessor):
    """Processor for the COMPQ Dataset"""

    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train-binary.tsv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test-binary.tsv")), "dev")
...
def compute_metrics(task_name, preds, labels):
    ...
    elif task_name == "compq":
        return rec_prec_f1(preds, labels)
    elif task_name == "mrpc":
...
def main():
    ...
    parser.add_argument('--load_finetuned_model', action='store_true', default=False, help="Load finetuned model.")
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if len(preds) == 0:
                ...
                softmax_preds[0] = np.append(softmax_preds[0], Softmax(1)(logits).detach().cpu().numpy(), axis = 0)
        ...
        with open(output_prediction_file, 'w') as writer:
            for i,pred in enumerate(softmax_preds):
                writer.write(str(pred[0]) + '\t' + str(pred[1]) + '\t' + eval_examples[i].text_a + '\n')
        ...
        # hack for MNLI-MM
        if task_name == "mnli":
            print("Skip MNLI dataset, needs to be handled separately")
